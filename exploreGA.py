...
def explore(n,child):
    for i in range(n):
        child[i] = bestNeighbour(n,child,i)
    return child
...
def geneticAlgorithm(population, populationSize, n):
    ...
    while True:
        ...
        if(randint(0,100)<=95):
            #Tell the best child to explore
            # print("Explore")
            exploredChild = explore(n,list(bestChild))
            exploredCost = cost(n,bestChild);
            if(exploredCost<=bestCost):
                print("Better explore")
                bestCost = exploredCost
                bestChild = exploredChild
                newPopulation.append(bestChild)

        if(bestCost==0 or exploredCost==0):
            if(bestCost==0):
                print("Return from here")
                return bestChild, bestCost
            if(exploredCost==0):
                print("Return explored")
                return bestChild, exploredCost
